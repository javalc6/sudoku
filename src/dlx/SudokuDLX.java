package dlx;
// DLX Sudoku solver, adapted to stop search on first result, if available
// reference: https://github.com/ShivanKaul/Sudoku-DLX/tree/master
...
public class SudokuDLX
{
 ...
 {        
  ...
  private boolean run(int[][] initialMatrix) 
  {
   ...
   return search(0); // start the Dancing Links process of searching and covering and uncovering recursively
  }
  ...
  {      
   ...
   ArrayList<int[]> cluesList = new ArrayList<>(); // the list used to get the clues. Because we use a raw ArrayList, we later have to cast to int[] before storing in clues
   int counter = 0; 
  ...
  private boolean search(int k) 
  {
   ...
   while(r != c) 
   {
    ...
    if (search(k+1)) //recursively search
		return true;//stop search on first result
...
//toString: pack board as string
 ...
 public static void main( String args[] ) throws Exception
 {
  ...
  SudokuDLX s = new SudokuDLX( puzzleSize );
